...
def trim_tree (tree):
    if len(tree) == 1:
        ...
        freq_count = tree[1]
        if type(freq_count) is str:
            #print(freq_count)
            return freq_count
        else:
            return (trim_tree(freq_count[0]), trim_tree(freq_count[1]))
...
def assign_bin_codes (node, binary_str=""):
    ...
    if type(node) is str:
        codes[node] = binary_str
        #print(codes[node])
    else:
        assign_bin_codes(node[0], binary_str+"0")
        assign_bin_codes(node[1], binary_str+"1")
